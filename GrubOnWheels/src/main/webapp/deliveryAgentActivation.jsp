...
<html lang="en">
    ...
    <body >
        <div class="container-scroller" >
            ...
            <script>alert('Delivery Agent Account Activated');</script>  
            <%}
            %>
            
            <%
                if (request.getParameter("Failed") != null) {%>
            <script>alert('Delivery Agent Account Activation/Rejection Failed! Please try again');</script>  
            <%}
            %>
            
            <%
                if (request.getParameter("Rejected") != null) {%>
            <script>alert('Delivery Agent Account Rejected');</script>  
            <%}
            %>
            <!-- partial -->
                                                    ...
                                                    <table class="table table-striped table-borderless">
                                                        ...
                                                        <tbody>
                                                            <%
                                                                ...
                                                                try {
                                                                    ResultSet rs = st.executeQuery("Select * from delivery_agent where status='InActive' order by id DESC");
                                                                    while (rs.next()) {

                                                            %>
                                                            <tr>
                                                                <td ><%=rs.getString("id")%></td>
                                                                <td ><img src="agentImg.jsp?delid=<%=rs.getString("id")%>" width="80px" height="80px" ></td>
                                                                ...
                                                                <td class="font-weight-bold"><%=rs.getString("status")%></td>
                                                                <td>
                                                                <form action="DeliveryAgentActivateReject" method="post">
                                                                <input type="hidden" name="id" value="<%=rs.getString("id")%>"/>
                                                                <input type="submit" name="delAction" value="Activate" class="btn btn-group-lg btn-success" onclick="DeliveryAgentActivateReject"/>
                                                                <input type="submit" name="delAction" value="Reject" class="btn btn-group-lg btn-danger" onclick="DeliveryAgentActivateReject"/>
                                                                </form></td>
                                                                <%                                        }
                                                                        rs.close();
                                                                        con.close();
