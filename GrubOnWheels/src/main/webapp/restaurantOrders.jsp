...
<html lang="en">
    ...
    <body>
                                                    ...
                                                    <table class="table table-striped table-borderless">
                                                        ...
                                                        <tbody>
                                                            <%
                                                                ...
                                                                try {
                                                                    ResultSet rs = st.executeQuery("Select * from useroders where resid='"+restid+"' and orderstatus='waiting' order by id DESC");
                                                                    while (rs.next()) {
                                                                    	String id = rs.getString("id");
                                                                    	String oid = rs.getString("oid");
                                                                    	ResultSet rs1 = st1.executeQuery("Select distinct resid from useroders where oid = '"+oid+"'");
                                                                    	int count = 0;
                                                                    	while(rs1.next()){
                                                                    		count++;
                                                                    	}
                                                            ...
                                                            <tr>
                                                                ...
                                                                <form action="RestaurantOrderAcceptReject" method="post">
                                                                <input type="hidden" name="id" value="<%=id%>"/>
                                                                <input type="hidden" name = "oid" value="<%=oid%>"/>
                                                                <input type="submit" name="restaction" value="Confirm" class="btn btn-group-lg btn-success"/>
                                                                <%if(count == 1){ %>
                                                                <input type="submit" name="restaction" value="Reject" class="btn btn-group-lg btn-danger"/>
                                                                <%} %>
                                                                </form>
                                                                </td>
                                                                <%                                        }
        ...
        <script>alert('Order Rejected');</script>  
        <%}
        %>
        
        <%
            if (request.getParameter("Failed") != null) {%>
        <script>alert('Order Update Failed! Try again!!');</script>  
        <%}
        %>
